...
\begin{align*}
  \etaunshriek_{pE\wt{\U}}(F,G) & \circ \I^{\omega}(\U) \\
    ...
    & = \etaunshriek_p(F,\wt{H})\circ \I_p(p) & \by{naturality of $\etaunshriek_{p,X,V}$ in $V$}.
\end{align*}
...
$\wt{u}_{1,\Idx(T)}(\J(\Gamma,T,P,s0)))$ for some $\J(\Gamma,T,P,s0)$ such that $\partial(\J(\Gamma,T,P,s0))=P$.
\end{construction}
%
\begin{remark}\rm
...
int(\Idx(\Gamma,F))$ be the morphism $\q(f,\Idx(\Gamma,F),3)$ defined\footnote{The notation $\q(\text{---,---},3)$
is defined in section 3 of \cite{Csubsystems}, by induction on the third
parameter.} by
...
%
by \ref{2015.04.04.eq1}\DG{verify this reference -- the equation there has changed} which equals, by construction,
