...
%
When we need to distinguish canonical squares arising from different universes we may
write $(X;F)_{p}$ and $f*_p g$.  We may also write $(X;F)'$ for $(X;F)_{p'}$.
%
\begin{remark}\rm
...
%
As is shown in \cite[Lemma 2.3]{Cfromauniverse}, the square
%
\begin{eq}
...
%
and we have constructed in \cite[Construction 2.6.4]{presheavesOb} a family of
...
$\etashriek_{p,X,V}(F,G)$ to $\etashriek_p(F,G)$; this should cause no
...
\end{align*}
%
be the morphisms introduced in \cite[(2.60)]{presheavesOb}. By \cite[(2.65)]{presheavesOb} we have
%
...
The first two squares are particular cases of \cite[Lemma
  4.1.3]{presheavesOb}, which defines the maps $\iota$ and $\iota'$.
To obtain the first one one has to set $Z=\U$,
...
%
\begin{definition}
\label{2015.03.27.def5} Let $p$ be a universe in $\C$ and $Eq$ be a
...
%
\begin{definition}
\label{2015.03.27.def6} A {\em J2-structure} on $p$, relative to a J0-structure $Eq$
...
\ref{2015.05.24.rem1}). The key to such a generalization is \cite[Remark
  3.13]{fromunivwithPi}\editorfootnote{The cited remark does not appear to have been published.}.
...
%
\begin{lemma}
\label{2015.05.14.l2} Let $p:E\sr B$ be in $FB$ and $f:B'\sr B$ be a
...
%
\begin{lemma}
\label{2015.05.14.l4} Let $B$ be fibrant and $p_2:E_2\sr E_1$, $p_1:E_1\sr B$
...
%
\begin{lemma}
\label{2015.05.14.l1} Assume that $\U,V$ are fibrant and that $p:\wt{\U}\sr \U$
...
%
\begin{lemma}
\label{2015.05.14.l3} Assume that $\U,V$ are fibrant and that $p:\wt{\U}\sr \U$
...
%
\begin{eq}
\label{2015.04.02.eq2} \q(f,int(\Gamma,F))=\Q(f,F)
\end{eq}%
%
...
%
\begin{eq}
\label{2015.03.27.eq7b} u_1:Ob_1(\Gamma)\sr Hom(int(\Gamma),\U)
\end{eq}%
%
\begin{eq}
\label{2015.03.27.eq7a} \wt{u}_1:\Obwt_1(\Gamma)\sr Hom(int(\Gamma),\wt{\U}),
\end{eq}%
%
