...
$IdxT(T)$ corresponding to the sequent-like expression
...
IdxT(T):=\Id_{\\p_T^*(T)}((\p_{\p_T^*(T)}^*(\delta(T))), \delta(\p_T^*(T)))
\end{eq}
%
We have
%
...
%
$$\delta(T)^*(IdxT(T))=\delta(T)^*(\Id_{\p_T^*(T)}((\p_{\p_T^*(T)}^*(\delta_T))),\delta(\p_T^*(T)))$$
...
%
$$int(IdxT(T))=int(\Id_{\p_T^*(T)}(o,o'))=(int(\p_T^*(T));(\wt{u}_1(o)*\wt{u}_1(o'))\circ
Eq)$$
%
where
%
