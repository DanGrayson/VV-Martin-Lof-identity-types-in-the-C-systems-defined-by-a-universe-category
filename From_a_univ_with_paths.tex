...
\begin{remark}
  ...
  For use below, we recall some notation from ${\gls{Ob}}_n(\Gamma)$ and
  $\gls{Obwt}_n(\Gamma)$ from \cite[\S 3]{fromunivwithPiI}, as well as
  $\gls{partial}(o)$ from \cite[\S 3]{Csubsystems}.  Let $\Gamma$ be an object
  in $\aCC$.  The set $\Ob_n(\Gamma)$ is the set of objects $\Delta$ of $\aCC$
  such that $l(\Delta)-n = \Gamma$ and $\ft^n(\Delta) = \Gamma$.
  ...
  Finally, for a map $g : Y \sr X$ such that $g \circ p_X = f$, we will let
...
\begin{remark}
  For $T\in Ob_1(\Gamma)$ we let $\gls{delta}(T)$ denote the
  morphism $T\sr \p_T^*(T)$, described equivalently\editorfootnote{The
    notation $\s_{\id_T}$, used here, is undefined.} as $\s_{\id_T}$ or
  as the only morphism such that $\delta(T)\circ \p_{\p_T^*(T)}=\id_T$ and
  $\delta(T)\circ \q(\p_T,T)=\id_T$.
\end{remark}
