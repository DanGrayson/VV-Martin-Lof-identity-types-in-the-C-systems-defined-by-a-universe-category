...
that there is an operation\DG{Should this be $LM\times R\times R\sr LM$ instead?}
%
...
\begin{remark}
  ...
  Further, if $s : \ft(X) \sr X$ is a section of $\p_X$, then $f^*(s)$ denotes
  ...
  the diagonal morphism $\delta(X) := \s_{\id_X} : X\sr \p_X^*(X)$.\DG{A natural question prompted by the terminology
    and the diagram below is whether $\q(\p_X,X) = \p_{\p_X^*(X)}$.}
  \begin{eq}
    \xymatrix@C=5pc{
...
\BA{Confused about the referee being addressed in the footnote.}
\editorfootnote{Dear referee, this is strange, since
...
%
Let $\C$ be a category\footnote{Not all of the constructions appearing in the sequel
...
\begin{align*}
  ...
  & =\Q'(g,\pr)\circ \Q'(\I^f(V),\pr') & \by{TODO}
\end{align*}
\BA{Proof is missing in the previous equation.}
...
We will obtain this equality as a consequence of commutativity of the three
