...
\cite{Warreninfty} and \cite{WarrenAwodey}.
...
\subsection{A note from the academic executor}

\newcommand{\editorfootnote}[1]{\footnote{Note from the academic executor: #1}}
...
involved, a footnote from the academic executor has been inserted.\editorfootnote{... such
  as this one.}
...
The academic executor thanks Benedikt Ahrens for many useful contributions to the editing
process.

\section{J-structures on C-systems and on universe categories}

We begin by recalling the definition of ``C-system'' from \cite{Csubsystems}.
C-systems were introduced by John Cartmell (\cite{Cartmell0},
\cite[p.\ 237]{Cartmell1}) and studied further by Thomas Streicher (see
\cite[Def. 1.2, p.\ 47]{Streicher}).  Both authors used the name ``contextual
categories'' for these structures.

By a pre-category $C$ we mean a pair of sets $Mor(C)$ and $Ob(C)$ with four maps
%
$$\partial_0,\partial_1:Mor(C)\sr Ob(C)$$
$$Id:Ob(C)\sr Mor(C)$$
%
and 
%
$$\circ:Mor(C)_{\partial_1}\times_{\partial_0} Mor(C)\sr Mor(C)$$
%
which satisfy the well known conditions of unity and associativity (note that we write composition of morphisms in the form $f\circ g$ or $fg$ where $f:X\sr Y$ and $g:Y\sr Z$). These objects would usually be called categories but we reserve the name ``category'' for those uses of these objects that are invariant under equivalence.  
%
\begin{definition}
A C0-system is a pre-category $CC$ with additional structure of the form
%
\begin{enumerate}
\item a function $\gls{l} : Ob(CC) \sr \nn$,
\item an object $pt$,
\item a map $\gls{ft}: Ob(CC)\sr Ob(CC)$, 
\item for each $X\in Ob(CC)$ a morphism $\gls{pX} : X \sr \ft(X)$,
\item for each $X\in Ob(CC)$ such that $l(X)>0$ and each morphism $f:Y\sr \ft(X)$ 
an object $\gls{fstarX}$  and a morphism $\gls{qfX} : f^*X \sr X$,
\end{enumerate}
%
...
\begin{enumerate}
\item $l^{-1}(0)=\{pt\}$
\item for $X$ such that $l(X)>0$ one has $l(\ft(X))=l(X)-1$
\item $\ft(pt)=pt$
\item $pt$ is a final object,
\item for $X\in Ob(CC)$ such that $l(X)>0$ and $f:Y\sr \ft(X)$ one has $l(f^*(X))>0$, $\ft(f^*X)=Y$ and the square
%
\begin{eq}\label{pb-square-C}
\begin{CD}
f^*X @>\q(f,X)>> X\\
@V\p_{f^*X}VV @VV\p_XV\\
Y @>f>> \ft(X)
...
%
commutes,
\item for $X\in Ob(CC)$ such that $l(X)>0$ one has $id_{\ft(X)}^*(X)=X$ and $\q(id_{\ft(X)},X)=id_X$,
\item for $X\in Ob(CC)$ such that $l(X)>0$, $g:Z\sr Y$ and $f:Y\sr \ft(X)$ one has $(gf)^*(X)=g^*(f^*(X))$ and $\q(gf,X)=\q(g,f^*X)\q(f,X)$.
\end{enumerate}
\end{definition}
%
\begin{remark}\rm
  In this definition $pt$ stands for ``point'' and serves as our notation for a final object of a category.
  The name ``ft'' stands for ``father'' which is the name given to this map in \cite[Def. 1.1]{Streicher}.
\end{remark}
%
...
For $f:Y\sr X$ in $CC$ we let $\ft(f):Y\sr \ft(X)$ denote the composition $f\circ \p_X$.
%
\begin{definition}
  A C-system is a C0-system together with an operation $f\mapsto \gls{sf}$ defined for all $f:Y\sr X$ such that $l(X)>0$ and such that the
  following properties hold.
%
\begin{enumerate}
\item $\s_f:Y\sr (\ft(f))^*(X)$,
\item $\s_f\circ \p_{(\ft(f))^*(X)} = \id_Y$,
\item $\s_f\circ \q(\ft(f),X) = f$,
\item if $X=g^*(U)$ where $g:\ft(X)\sr \ft(U)$ then $\s_f=\s_{f\circ \q(g,U)}$.
\end{enumerate}
\end{definition}

From \cite[Prop.~2.4]{Csubsystems} it follows that, in a C-system, the squares \ref{pb-square-C} are pullback squares,
and the maps $\s_f$ are obtainable using the universal property of those pullback squares.
\begin{remark}\rm
  We record some identities for later reference.

  In the case where $Y = \ft(X)$ and $\ft(f) = 1_X$, and thus $f$ is a section of $\p_X$,
  one has the following identity.
  \begin{eq}
    \label{section-section}
    \s_f = f
  \end{eq}%
  The formation of the section $\s_f$ is natural in the sense that, for a map
  ...
  In the case where one takes $Y=X$, and $f = \id_X$, we let $\gls{deltaX}$ denote
  ...
  \end{eq}%
  Using \eqref{s-naturality}, one shows, for a map $f : Y \to X$, the following identity.
  \begin{eq}
  ...
  notation $\gls{parto}$ from \cite[\S 3]{Csubsystems}.
  ...
  Observe that $\s_f$ above is an element of $\Obwt_1(Y)$, and $\delta(X)$ above
...
\cite{Jacobs1}, \cite[Remark 2.4.1]{GambinoGarner}, and \cite{vandenBergGarner2011},
...
\label{univstrdiag}
\]
for all objects $X$ and all morphisms $F:X\sr \U$.
...
Recall that for an object $X$ of $\C$, the \emph{slice} category $\C/X$ is the
category whose objects are morphisms $p : Y \to X$ of $\C$, and whose morphisms
from $p : Y \to X$ to $p' : Y' \to X$ are those morphisms $f : Y \to Y'$ that
make the evident triangle commute.  We may also use $(Y,p)$ as notation for the
object $p$, to prevent ambiguity.  When $\C$ is locally Cartesian closed, then
the slice categories are Cartesian closed, and we use $\iHom((Y,p), (Y',p'))$
to denote the internal Hom in the slice category.

When $\C$ is a locally Cartesian closed category, any morphism
$p:\wt{\U}\sr \U$ defines a functor
$$\gls{Ip} : \C \to \C/\U$$ which sends an object $V$ to 
$$\I_p(V) := \iHom((\wt{\U},p),(\U\times V,\pr_1)).$$
We denote by
...
\ref{2015.05.24.rem1}). 
%%                         The key to such a generalization is \cite[Remark
%%   3.13]{fromunivwithPi}
%%   DG: The remark cited does not appear in the paper cited -- the numbering in section
%%   3 stops with 3.12.  So I've removed this sentence.
...
\end{xy}
$$
Here the bottom horizontal arrow is the projection to $\U$.
%
...
functors between categories with two universes, which we will apply in section \ref{univ-comp-J} to the
...
The following theorem will be used to formulate the condition of compatibility
of a universe functor with full J-structures.
...
\subsection{Universe category functors compatible with J-structures}\label{univ-comp-J}
...
$\chi_{{\bf\Phi}_{E}}(E\wt{\U})\circ \I_{pE\wt{\U}'}(\wt{\phi}_{E})$.

Applying Theorem \ref{2015.04.10.th3} in this context we get the following.
%
\begin{theorem}
...
$(\Phi,\phi,\wt{\phi})$ is said to be \emph{compatible} with $Jp$ and $Jp'$ if it is
...
In Lemma \ref{2015.04.12.l1} we will use the definition above to show that
universe category functors compatible with J-structures give rise to
homomorphisms of C-systems with J-structures.
