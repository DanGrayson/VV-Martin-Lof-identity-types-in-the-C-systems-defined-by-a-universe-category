...
write $(X;F)_{p}$ and $f*_p g$.  We may also write $(X;F)'$ for $(X;F)_{p'}$ and $(X;F)'_i$ for $(X;F)_{p'_i}$.
%
\begin{remark}\rm
...
\end{align}
%
For any map $f:X' \sr X$, we will write $\gls{QfFE}$ for the canonical morphism from $(X;f\circ
F)_{E}$ to $(X;F)_{E}$.
%
\begin{definition}
...
We will use the notion of \emph{functor of universe categories}, defined in \cite[4.1]{Cfromauniverse}
and in \cite[\S 5]{fromunivwithPi}.
...
$$
%
and $f:\wt{\U}'\sr \wt{\U}$ over $\U$, we let $\gls{Fstarf}$ denote the unique morphism
...
Moreover, in \cite[Construction 5.2]{fromunivwithPi} we have defined a map
$\gls{Phi2} : \D_{p}(X,V) \sr \D_p(\Phi(X),\Phi(V))$, which we will use later.
...
\item the triples ${\bf\Phi}_i:=(\Phi,\phi,\wt{\phi}_i)$, $i=1,2$ are universe
  category functors, in particular, $\Phi$ takes canonical squares of $p_1$ and $p_2$ to
...
\end{align*}
Here (and below) $\iota_i$ is the canonical isomorphism of type $(\Phi(X) ; \Phi(F_1) \circ \phi)'_i \xrightarrow\cong \Phi((X;F_1)_i)$, for $i=1,2$,
that results from the two objects being pullbacks of the same diagram, as illustrated here:
\begin{eq}
  \label{iota-diagram}
  \begin{xy}
    \xymatrix@C=6pc{
      (\Phi(X);\Phi(F_1)\circ\phi)'_i
      \ar@/^1.5pc/[rrrd]^{\Q(\Phi(F_1)\circ\phi)}
      \ar@/_1.5pc/[rdd]_{\p'_{i,\Phi(X),\Phi(F_1)\circ\phi}}
      \ar[rd]^{\iota_i}_{\cong}                 \\
      & \Phi((X;F_1)_i) \ar[r]_-{\Phi(\Q_i(F_1))} \ar[d]^{\Phi(\p_{i,X,F_1})} & \Phi(\wt\U_i) \ar[r]_{\wt\phi_i} \ar[d]^{\Phi(p_i)} & \wt\U'_i \ar[d]^{p'_i} \\
      & \Phi(X) \ar[r]_{\Phi(F_1)} & \Phi(U) \ar[r]_{\phi} & \U'
      }
    \end{xy}
\end{eq}
...
\begin{align*}
  ...
  &  =(\Phi(F_1)\circ \phi)^*(g')\circ \Q_2(\Phi(F_1)\circ \phi) \\
  &  =\Q_1(\Phi(F_1)\circ\phi)\circ g',
\end{align*}
where the first equality follows from the definition of $\iota_2 = \iota$ in
\cite[Construction 5.2]{fromunivwithPi}, and the second follows from
(\ref{2015.04.08.eq3}). On the other hand,
%
\begin{align*}
  \iota_1\circ & \Phi(F_1^*(g))\circ \Phi(\Q_2(F_1))\circ\wt{\phi}_2 \\
    & = \iota_1\circ \Phi(F_1^*(g)\circ \Q_2(F_1))\circ \wt{\phi}_2 & \by{$\Phi$ being a functor} \\
    & = \iota_1\circ \Phi(\Q_1(F_1)\circ g)\circ \wt{\phi}_2 & \by{\ref{2015.04.08.eq3}} \\
    & = \iota_1\circ \Phi(\Q_1(F_1))\circ \Phi(g)\circ \wt{\phi}_2 & \by{$\Phi$ being a functor} \\
    & = \iota_1\circ \Phi(\Q_1(F_1))\circ \wt{\phi}_1\circ g' & \by{\ref{two-univ-comm}} \\
    & = \Q_1(\Phi(F_1)\circ \phi)\circ g' & \by{iota-diagram}
\end{align*}
%
This proves the first equation. For the second equation we have:
%
$$(\Phi(F_1)\circ \phi)^*(g')\circ \iota_2\circ
