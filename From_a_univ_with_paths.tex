...
bijections\editorfootnote{since $\I_p(V)$ is an object over $\U$, one may wonder what $Hom(X,\I_p(V))$ means here}
%
...
Let $Eq$ be a J0-structure on $p$. Consider the object\editorfootnote{$(\wt{\U};p,Eq)$ is
  notation for $((\wt{\U};p);Eq)$.}
$$E\wt{\U}:=(\wt{\U};p,Eq)$$
...
Let us show first that\editorfootnote{$\etaunshriek_p$ and $\etaunshriek_{pE\wt{\U}}$ are undefined here, and they occur later, too.}
%
...
%
Consider the element\editorfootnote{$\etashriek_{pE\wt{\U}}$ is undefined here}
%
...
\ref{2015.04.04.constr1} we have\editorfootnote{$\etaunshriek_{E\wt{\U}}$ is undefined here}:
%
