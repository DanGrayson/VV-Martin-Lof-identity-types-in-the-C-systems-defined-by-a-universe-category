...
regular sub-quotient of a C-system of the form $CC(R,LM)$ where $R$ is a monad
...
$CC(R,LM)$ our notation only needs to provide a reference to elements of sets
associated with $CC(R,LM)$ itself.  There, the first $T$ refers to an element
...
%
to define an element of $Ob(CC(LM,R))$, the expression $IdT^s\,T\,x\,y$ must
...
J-structure on the C-system $CC({\mathcal C},p)$ from a J-structure on $p$.

Combined together they provide a method of constructing a J-structure on
$CC({\cal C},p)$ from a J1-structure on $p$.
...
on categories $CC({\cal C},p)$ first appeared in \cite{CMUtalk}.
...
Let $IdT$ be a J0-structure on $CC$. A J1-structure over $IdT$ is a family of
functions
%
...
if for all $T\in Ob_{\ge 1}(CC)$ and $o,o'\in \wt{Ob}(T)$ one has\footnote{The
...
%
The construction of a C-system $CC({\mathcal C},p)$ from a category with a
...
notations. The underlying category of $CC({\mathcal C},p)$ is equipped with a
functor $int$ to $\mathcal C$. Note that while $int$ is the identity on
morphisms by construction of $CC({\mathcal C},p)$, the notations for the same
...
\end{construction}
%
We let $IdT_{Eq}$ denote the J0-structure on $CC({\mathcal C},p)$ constructed
...
C$. To construct, for all objects $\Gamma\in Ob(CC({\mathcal C},p))$ two maps
%
...
$p$. To construct for all $\Gamma\in Ob=Ob(CC({\mathcal C},p))$, for all $T\in
...
$\wt{Ob}(CC({\mathcal C},p))$ that satisfies the equation
%
...
$f:\Gamma'\sr \Gamma$ a morphism in $CC({\mathcal C},p)$ and $F:int(\Gamma)\sr
...
$p$. To construct a J-structure on $CC({\mathcal C},p)$ relative to $IdT_{Eq}$
and $refl_{\Omega}$.
\end{problem} 
%
...
%
\begin{enumerate}
\item Let $IdT$, $IdT'$ are J0-structures on $CC$ and $CC'$ respectively.  Then
  $H$ is called a homomorphism of C-systems with J0-structures $(CC,IdT)\sr
  (CC,IdT')$ if for each $\Gamma\in Ob(CC)$ and $o,o'\in\wt{Ob}_1(\Gamma)$ such
...
\item Let $IdT$, $IdT'$ be as above and let $refl$, $refl'$ be J1-structures
  ...
  J0-structures $H:(CC,IdT)\sr (CC',IdT')$ is called a homomorphism of
...
\end{definition}
%
For a C-system $CC$ with a J0-structure $IdT$ and a J1-structure $refl$ over
$IdT$ define $Jdom(CC,IdT,refl)$ as the set of quadruples $(\Gamma,T,P,s0)$
where $\Gamma\in Ob$, $T\in Ob_1(\Gamma)$, $P\in Ob_1(IdxT(T))$ and $s0\in
\wt{Ob}(rf_T^*(P))$. Equivalently we can say that $Jdom(CC,IdT,refl)$ is the
...
%
if for all $\Gamma\in Ob(CC)$, $T\in Ob_1(\Gamma)$, $P\in Ob_1(IdxT(T))$ and
...
Ob_n(CC({\mathcal C},p))$, pairs $(H(\Gamma),\psi_{\Gamma})$ where
$H(\Gamma)\in Ob(CC({\mathcal C}',p'))$ and $\psi_{\Gamma}$ is a morphism
%
...
$\Gamma\in Ob(CC({\mathcal C},p))$ and $o,o'\in \wt{Ob}_1(\Gamma)$ such that
$\partial(o)=\partial(o')$ one has
%
...
%
We need to check that for $\Gamma\in Ob(CC({\mathcal C},p))$ and $s\in
...
$\Gamma\in Ob(CC({\mathcal C},p))$ be such that $l(\Gamma)\ge n$. Then the
square
%
$$
\begin{CD}
...
with $Eq$ and $Eq'$. The for all $\Gamma\in Ob(CC({\mathcal C},p))$, $T\in
...
We need to verify that for all $\Gamma\in Ob(CC({\mathcal C},p))$, $T\in
