...
\newcommand{\Idx}{\mathsf{Id_x}} % originally was "IdxT"
...
\newoperator{deepartial}{\partial}
\newoperator{delta}{\delta}
\newoperator{D}{\D}
\newoperator{etashriek}{\eta^!}
\newoperator{eta}{\eta}
\newoperator{Fp}{\Fp}
\newoperator{ft}{\ft}
\newoperator{Idx}{\Idx}
...
$\Id$, and $\J$ is a J2-structure over $\refl$.  For the notation used below,
including ${\gls{Ob}}_n(\Gamma)$ and $\gls{Obwt}_n(\Gamma)$, see
\cite[\S2]{fromunivwithPi}, as well as \cite[\S3]{Csubsystems} for $\gls{deepartial}(o)$.
%
\begin{definition}
...
Recall that for $T\in Ob_1(\Gamma)$ we let
$\gls{delta}(T)$ denote the morphism $T\sr \p_T^*(T)$ that can be described
...
%
\begin{align*}
  \delta(T)^*(\Idx(T)) & = \delta(T)^*(\Id_{\p_T^*(T)}((\p_{\p_T^*(T)}^*(\delta_T))),\delta(\p_T^*(T)))  & \by{...}\\
    & = \Id_T(\delta(T)^*(\p_{\p_T^*(T)}^*(\delta_T)),\delta(T)^*(\delta(\p_T^*(T)))) & \by{...}\\
...
For $f:W\sr X$ and $g:W\sr \wt{\U}$ we will denote by $\gls{starfg}$ the unique morphism
such that
%
\begin{align*}
  (f*g)\circ \p_{X,F} & =f \\
...
%
$$\gls{eta}_{p,X,V}:\D_p(X,V)\sr Hom(X,\I_p(V))$$
%
...
$$
%
where the left hand side arrow is of the form\editorfootnote{What is $D^f$ (above and below)?}
%
...
\end{eq}
%
%$\psi(F,\wt{G})\in \wt{Ob}_1(\Idx(\Gamma,F))$.
\end{problem}
%
...
%
\begin{align*}
  \eta_{pE\wt{\U}}(F,G)\circ I^{\omega}(\U)
    & = \eta_p(F,G\circ F^*(\omega)) \\
    & = \eta_p(F,\rf_T\circ G) \\
    & = \eta_p(F,s0\circ Q(\rf_T\circ G)\circ p) \\
    & = \eta_p(F,\wt{H}\circ p) \\
    & = \eta_p(F,\wt{H})\circ \I_p(p),
\end{align*}
%
...
%
\begin{align*}
  f\circ \eta_{pE\wt{\U}}(F,\wt{u}_1(\J))
    & = \eta_{pE\wt{\U}}(f\circ F, \wt{u}_1(Q(f,F)_{E}^*(\J,1))) \\
...
%
\begin{align*}
  \eta'({\bf\Phi}_2^2&(\eta^{-1}(Id_{X})))\circ I^{g'}(\Phi(V)) \\
    & = \eta(D^{g'}(\Phi(X),\Phi(V))({\bf\Phi}_2^2(\eta^{-1}(Id_X)))) \\
    & = \eta({\bf\Phi}_1^2(D^g(X,V)(\eta^{-1}(Id_X))))
\end{align*}
%
...
\begin{align*}
  \Phi(a)\circ & \chi_1(V) \\
    & = \Phi(a)\circ \eta'({\bf\Phi}^2_1(\eta^{-1}(Id_{\I_{p_1}(V)}))) \\
    & = \eta'(\D_{\p_1'}(\Phi(a),\Phi(V))({\bf\Phi}^2_1(\eta^{-1}(Id_{\I_{p_1}(V)}))),
\end{align*}
%
...
%
\begin{align*}
  \eta_{pE\wt{\U}}(&u'_1(H(T)),\wt{u}'_{1,{IdxT'(H(T)}}(H(\J)))) \\
    & = \eta_{pE\wt{\U}'}(\D_{pE\wt{\U}'}(\psi_{\Gamma},\_)(\D_{pE\wt{\U}'}(\_,\wt{\phi})(\Phi_E^2(u_1(T),\wt{u}_{1,\Idx(T)}(\J))))) \\
    & = \psi_{\Gamma}\circ \eta_{pE\wt{\U}'}(\Phi_E^2(u_1(T),\wt{u}_{1,\Idx(T)}(\J)))\circ \I_{pE\wt{\U}'}(\wt{\phi})
\end{align*}
%
...
\begin{align*}
  \psi_{\Gamma}\circ \eta_{pE\wt{\U}'}&(\Phi_E^2(u_1(T),\wt{u}_{1,\Idx(T)}(\J)))\circ \I_{pE\wt{\U}'}(\wt{\phi}) \\
    & = \psi_{\Gamma}\circ \Phi(\eta_{pE\wt{\U}}(u_1(T),\wt{u}_{1,\Idx(T)}(\J)))\circ \chi_{{\bf\Phi}_E}(\wt{\U})\circ \I_{pE\wt{\U}'}(\wt{\phi}) \\
...
\begin{align*}
  \psi_{\Gamma}\circ \Phi&(\phi(\Gamma,T,P,s0))\circ R_{\Phi}\circ pr_1'  \\
    ...
    & = \psi_{\Gamma}\circ \Phi(\eta_{pE\wt{\U}}(F,G))\circ \zeta_{\Phi}
\end{align*}
%
Continuing we get
%
\begin{align*}
  \psi_{\Gamma}\circ & \Phi(\eta_{pE\wt{\U}}(F,G))\circ \zeta_{\Phi} \\
    & = \psi_{\Gamma}\circ \Phi(\eta_{pE\wt{\U}}(F,G))\circ \xi_{{\bf \Phi}_E}\circ \I_{pE\wt{\U}'}(\phi) \\
...
\begin{align*}
  \psi_{\Gamma}\circ&\eta_{pE\wt{\U}'}({\bf\Phi}^2_E(F,G))\circ \I_{pE\wt{\U}'}(\phi) \\
    & = \eta_{pE\wt{\U}'}(\D_{pE\wt{\U}}(\psi_{\Gamma},\_)(\D_{pE\wt{\U}}(\_,\phi)({\bf\Phi}^2(F,G)))) \\
    & = \eta_{pE\wt{\U}'}((u_{1,H(\Gamma)}'(H(T)),u_{1,IdxT'(H(T))}(H(P)))),
\end{align*}
%
...
\begin{align*}
  \psi_{\Gamma}\circ & \Phi(\phi(\Gamma,T,P,s0))\circ R_{\Phi}\circ pr_2'  \\
    ...
    & = \psi_{\Gamma}\circ \Phi(\eta_p(F,\wt{H}))\circ \wt{\xi}_{\Phi} \\
    & = \eta_{p'}(\wt{u}_{2,H(\Gamma)}'(H(s0))),
\end{align*}
%
