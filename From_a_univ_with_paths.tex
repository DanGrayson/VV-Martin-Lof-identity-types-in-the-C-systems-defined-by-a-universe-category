...
\item the triples ${\bf\Phi}_i:=(\Phi,\phi,\wt{\phi}_i)$, for $i=1,2$, are universe
  category functors from $(\C,p_i)$ to $(\C',p'_i)$.
\end{enumerate}
...
%
$$\zeta_i:\Phi(\I_{p_i}(\U))\sr \I_{p'_i}(\U')$$
%
given by $\zeta_i=\chi_i(\U)\circ \I_{p'_i}(\phi)$ and
%
$$\wt{\zeta}_i:\Phi(\I_{p_i}(\wt{\U}_1))\sr \I_{p'_i}(\wt\U'_1)$$
%
...
(This isomorphism appeared above, as the inverse $\Phi_{X,F} = \iota_{{\bf\Phi},X,F}^{-1}.)$
Let $\Phi\wt{\U}p$ be the composition
%
...
Eq'$$
%
and by \cite[Lemma 6.1(2)]{fromunivwithPi} we have
%
\begin{align*}
