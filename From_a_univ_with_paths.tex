...
from $p : Y \to X$ to $p' : Y' \to X$ are those morphisms $f : Y \to Y'$ that make the
evident triangle commute.  When $\C$ is locally Cartesian closed, then the
slice categories are Cartesian closed, and we use $\iHom((Y,p), (Y',p'))$ to
denote the internal Hom in the slice category.
