...
functor $\gls{int}$ to $\C$. Note that while $int$ is the identity on
Hom-sets by the construction of $\toCC({\C},p)$, the notations for an
element of $Hom(\Gamma',\Gamma)$ and $Hom(int(\Gamma'),int(\Gamma))$ may
differ.  In particular, for $f:\Gamma'\sr \Gamma$ and $F:\Gamma\sr \U$ in $\toCC({\C},p)$, we have the following
equation.\editorfootnote{The
  ...
  the right side, $\Q(f,F)$, refers to the universe structure on $\C$.}
%
\begin{eq}
